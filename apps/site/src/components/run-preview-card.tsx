const STEPS = [
  ...
  { label: "Ação proposta", detail: "Atualizar status do ticket #4821 → Aprovado", status: "done" },
...
export function RunPreviewCard() {
  return (
    <div className="w-full max-w-sm rounded-lg border border-border bg-surface p-5 shadow-[0_4px_16px_rgba(0,0,0,0.5)]">
          ...
          <li key={step.label} className="flex gap-3">
            <span
              className={
                ...
                (step.status === "done" ? "bg-success" : "bg-warning")
              }
              aria-hidden
            />
